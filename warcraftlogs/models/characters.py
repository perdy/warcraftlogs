...
class Class(metaclass=BaseMeta):
    ...
    def _get_specs_from_dict(cls, data, default=None):
        return {s.id: s for s in [Spec.from_dict(i) for i in data.get(cls.attributes['specs'], [])]}
...
class Character(metaclass=BaseMeta):
    ...
    def _get_talents_from_dict(cls, data, default=None):
        return {t.id: t for t in [Talent.from_dict(i) for i in data.get(cls.attributes['talents'], [])]}

    @classmethod
    def _get_gear_from_dict(cls, data, default=None):
        return {g.id: g for g in [Gear.from_dict(i) for i in data.get(cls.attributes['gear'], [])]}
