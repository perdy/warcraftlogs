...
class WarcraftLogsClient:
    ...
    def rankings_encounter(self, encounter_id, **params):
        ...
        return [
            Fight.from_dict(f, encounter=encounter_id)
            for f in self._get(path, **params).json()["rankings"]
        ]

    def rankings_character(self, name, server, region, **params):
        path = "rankings/character/{}/{}/{}".format(name, server, region)
        return [
            Fight.from_dict(f, name=name, server=server, region=region)
            for f in self._get(path, **params).json()
        ]
